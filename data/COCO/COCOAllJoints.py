...
class COCOJoints(object):
    def __init__(self):
        ...
        for mpi, stage in zip([self.mpi, self.test_mpi], ['train', 'val']):
            if stage == 'train':
                self._train_gt_path=os.path.join(cur_dir, 'MSCOCO', 'person_keypoints_trainvalminusminival2014.json')
                coco = COCO(self._train_gt_path)
            else:
                self._val_gt_path=os.path.join(cur_dir, 'MSCOCO', 'person_keypoints_minival2014.json')
                coco = COCO(self._val_gt_path)
            if stage == 'train' or 'val':
                for aid in coco.anns.keys():
                    ...
                    if np.sum(joints[2::3]) == 0 or ann['num_keypoints'] == 0 :
                        continue
                    imgname = 'MSCOCO/' + prefix + '2014/' + 'COCO_' + prefix + '2014' + '_' + str(ann['image_id']).zfill(12) + '.jpg'
                    img_path = osp.join(osp.abspath(cur_dir), imgname)
                    humanData = dict(aid = aid, joints=joints, imgpath=img_path, headRect=rect, bbox=bbox, imgid = ann['image_id'], segmentation = ann['segmentation'])
